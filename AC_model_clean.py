...
main_window = pyglet.window.Window(width=int(VARS.SCREEN_WIDTH*.685),
	height=int(VARS.SCREEN_HEIGHT*.94),resizable=True,
...
rule_plot_window = pyglet.window.Window(width=int(VARS.SCREEN_WIDTH*.31),
	height=int(VARS.SCREEN_HEIGHT*.47),resizable=True,
...
product_plot_window = pyglet.window.Window(width=int(VARS.SCREEN_WIDTH*.31),
	height=int(VARS.SCREEN_HEIGHT*.44),resizable=True,
...
AC_Sprites.Control_Sprite(pyglet.image.load("oval.png"), VARS,
	int(VARS.SCREEN_WIDTH*.39), 3,.25,
	"active", control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("counts.png"), VARS,
	int(VARS.SCREEN_WIDTH*.44), 1,.38,
	"counts", control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("Networks.png"),VARS,
	int(VARS.SCREEN_WIDTH*.48),1,.38,
	"network",control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("labels.png"),VARS,
	int(VARS.SCREEN_WIDTH*.52), 1,.38,
	"labels",control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("rewind.png"),VARS,
	int(VARS.SCREEN_WIDTH*.56), 1,.38,
	"backward",control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("pause.png"),VARS,
	int(VARS.SCREEN_WIDTH*.60),1,.38,
	"pause",control_batch,main_window)
AC_Sprites.Control_Sprite(pyglet.image.load("play.png"),VARS,
...
def on_draw():
	...
	control_batch.draw()
	pyglet.text.Label("Active",x=int(VARS.SCREEN_WIDTH*.398),
		y=20, color=(0,0,0,150),font_size=10).draw()
...
def on_draw():
	...
	if viz:
		...
		pyglet.graphics.draw(2, pyglet.gl.GL_LINES,
			('v2f', (VARS.SCREEN_WIDTH*.25,VARS.SCREEN_HEIGHT*.455,
			VARS.SCREEN_WIDTH*.30,VARS.SCREEN_HEIGHT*.455)),
			('c3B',(50,50,255,150,150,150)))
		pyglet.text.Label("<----------", x=VARS.SCREEN_WIDTH*.255,
			y=VARS.SCREEN_HEIGHT*.435, color=(0,0,0,150),
			font_size=10, bold=True).draw()

		pyglet.graphics.draw(2, pyglet.gl.GL_LINES,
			('v2f', (VARS.SCREEN_WIDTH*.25,VARS.SCREEN_HEIGHT*.42,
			VARS.SCREEN_WIDTH*.30,VARS.SCREEN_HEIGHT*.42)),
			('c3B',(50,50,255,50,50,255)))

		pyglet.text.Label("<--------->", x=VARS.SCREEN_WIDTH*.255,
			y=VARS.SCREEN_HEIGHT*.40, color=(0,0,0,150),
			font_size=10, bold=True).draw()

	pyglet.text.Label("Rules: "+str(VARS.RULE_COUNT),
		x=VARS.SCREEN_WIDTH*.235,y=VARS.SCREEN_HEIGHT*.01,
		...
		for prod,count in prods.items():
			if count > 0:
				...
				if count > VARS.PRODUCT_COUNT:
					count = float("inf")
				pyglet.text.Label(str(prod) +": "+str(count),x=x+x1+VARS.X_INC/3,
					y=y+y1+VARS.Y_INC/2,color=(0,0,0,255),
					font_size=14, bold=True).draw()
...
def update_product_count():
	...
	if VARS.HOMOGENEOUS:
		cnts = {}
		for product in range(1,VARS.TYPES+1):
			cnts[product] = len(VARS.URN.collection[product])

		VARS.PRODUCT_COUNTS = {(0,0):cnts}
	else:
		pass
...
for i in range(VARS.CELL_COUNT):
	...
	sprite.scale = 3.19/VARS.SPACE_WIDTH
